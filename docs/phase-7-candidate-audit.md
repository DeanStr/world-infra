...
| Redis/in-memory idempotency backends | Deferred in Phase 7; superseded post-plan | Airline's Redis idempotency is runtime-critical and incarnation-scoped; Chairman's idempotency is mostly SQL ledger/request handling. | Phase 7 kept `idempotency-core` focused on key construction/validation. The later `docs/rfcs/idempotency-runtime-core.md` RFC approves a volatile runtime claim-store crate with Chairman adoption deferred. |
...
runtime/provider backends. The later `idempotency-runtime-core` crate is a
post-plan RFC exception, not Phase 7 scope.
