...
| Redis/in-memory idempotency backends | Defer | Airline's Redis idempotency is runtime critical and incarnation-scoped; Chairman's idempotency is primarily SQL ledger/request handling. A shared backend would either be Airline-shaped or too abstract. | Keep `idempotency-core` to key construction/validation for now. |
